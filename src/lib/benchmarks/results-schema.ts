...
export const BenchmarkQuestionResultSchema = z.object({
  ...
  scoring: z.enum(["positive_agreement", "ordered_mean"]).optional(),
  unweightedN: z.number().int().nonnegative(),
...
export const BenchmarkResultSnapshotSchema = z.object({
  ...
  execution: z
    .object({
      ...
      sampleStrategy: z.string().min(1).optional(),
      questionCount: z.number().int().positive().optional(),
      directEstimateCalls: z.number().int().nonnegative().optional(),
      simulatedResponseCalls: z.number().int().nonnegative().optional(),
    })
    .optional(),
