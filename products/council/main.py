...
def render():
    ...
    if st.session_state.step == 1:
        if st.button(
            ...
            if not question.strip():
                ...
                input_tokens_est = (
                    advisor_system_tokens
                    + advisor_user_tokens
                    + summarizer_input_tokens
                )

                output_tokens_est = (
                    advisor_output_tokens + summarizer_output_tokens
                )

                input_tokens_cost = (
                    input_tokens_est * MODEL_COST_MAP[model_type].Input / 1e6
                )
                output_tokens_cost = (
                    output_tokens_est * MODEL_COST_MAP[model_type].Output / 1e6
                )
    ...
    # Step 2: Run Simulation
        ...
        st.write(
            f"**Tokens:** {cost_data['input_tokens_est']} input, "
        ...
        if st.button(
            "Get Advice",
            help="Click to start simulation with the current settings.",
        ):
